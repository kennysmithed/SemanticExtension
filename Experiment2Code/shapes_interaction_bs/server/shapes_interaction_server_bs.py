...
# block 1 establishes colour associations (NB this block is double the size of subsequent blocks)
# block 2 involves discriminating between colour splats
# then block 3 is ONE OF
# i) discriminating between shapes based on colour (equivalent to v1 block 2)
# ii) objects
# iii) emotions
...
break_trials = [72] #list of trials to offer break after: 72 = block 0 (8) + block 1 (64); whole experiment should be 136
...
#each pair will select n_shapes_per_pair from this list - NB these shapes are abstract (integers) and will be mapped 
#onto an different shape image (from ['square','circle','diamond','star', 'cross', 'pentagon', 'hexagon'])
#at each client, which allows us to show different shapes to different members of a pair. 
all_shapes = range(7)
all_actual_shapes = ['square','circle','diamond','star', 'cross', 'pentagon', 'hexagon']
...
# block3 is coloured_shapes, objects or emotions 
...
def trial_list(block3_condition,shapes,colour_shape_correspondences):
    ...
    for _i in range(1): #just 1 rep for block0 because it is so easy
        ...
        for target_shape in shapes:
            ...
            target = {'shape':target_shape,'colour':"white"}
            foils = [{'shape':foil_shape,'colour':"white"} for foil_shape in foil_shapes]
    ...
    for _i in range(n_subblocks_per_block*2): #NEW in v3 - doubling length of this block
        ...
        for target_shape in shapes:
            ...
            target = {'shape':target_shape,'colour': target_colour}
            foils = [{'shape':foil_shape,'colour': foil_colour} for foil_shape,foil_colour in zip(foil_shapes,foil_colours)]
    ...
    for _i in range(n_subblocks_per_block):
        ...
        for target_colour in colours:
            ...
            target = {'shape':"splat",'colour':target_colour}
            foils = [{'shape':"splat",'colour':foil_colour} for foil_colour in foil_colours]
    ...
    # NB doing this for *all* colours
    block3 = []
    if block3_condition == 'coloured_shapes':
        for _i in range(n_subblocks_per_block):
            ...
            for target_colour in colours:
                ...
                foil_colours = n_random_with_taboo(colours,n_foils,[target_colour])
                target = {'shape':target_shape,'colour': target_colour}
                foils = [{'shape':target_shape,'colour':foil_colour} for foil_colour in foil_colours]
    ...
    elif block3_condition == 'objects':
        # easy
        for _i in range(n_subblocks_per_block):
            this_block3_subblock = []
            for target_object in objects:
                foils = n_random_with_taboo(objects,n_foils,[target_object])
                this_block3_subblock.append({'target':target_object,'foils':foils,'block':3})
            block3 = block3 + shuffle(this_block3_subblock) #using + to get flat structure

    elif block3_condition == 'emotions':
        # easy
        for _i in range(n_subblocks_per_block):
            this_block3_subblock = []
            for target_emotion in emotions:
                foils = n_random_with_taboo(emotions,n_foils,[target_emotion])
                this_block3_subblock.append({'target':target_emotion,'foils':foils,'block':3})
            block3 = block3 + shuffle(this_block3_subblock) #using + to get flat structure
...
#shuffles list_to_shuffle such that no positions have same values as list_to_avoid
def shuffle_distinct(list_to_shuffle,list_to_avoid):
    assert len(list_to_shuffle) == len(list_to_avoid)
    candidate_shuffle = list_to_shuffle
    satisfactory_shuffle = False
    while not(satisfactory_shuffle):
        candidate_shuffle = shuffle(list_to_shuffle)
        matches = [candidate_shuffle[i]==list_to_avoid[i] for i in range(len(candidate_shuffle))]
        if not(any(matches)):
            satisfactory_shuffle=True
    return candidate_shuffle
...
def enter_phase(client_id,phase):
    ...
    # Attempts to pair this client with anyone already in the waiting room
        ...
        if (len(unpaired_clients)%2 ==0): #If there are exactly 2 people now in unpaired_clients, pair them
            ...
            #random condition (where condition = fixed or random)
            ...
            #give out fixed more than random
            #this_pair_condition = random.choice(["fixed_associations","fixed_associations","random_associations"])
            
            #random assignment to one of these options for block 3
            this_pair_block3 = random.choice(["coloured_shapes","objects","emotions"])
            #restricted choice with e.g. 
            #this_pair_block3 = random.choice(["coloured_shapes","objects"])

            print(this_pair_condition)
            print(this_pair_block3)
            ...
            # generate a distinct randomisation of the shapes list for each participant, 
            # to be used in mapping the abstract shapes to actual shapes at the client side
            mapping1 = shuffle(all_actual_shapes)
            mapping2 = shuffle_distinct(all_actual_shapes,mapping1)
            ...
            trials1 = trial_list(this_pair_block3,this_pair_shapes,this_pair_correspondences)
            trials2 = trial_list(this_pair_block3,this_pair_shapes,this_pair_correspondences)
            ...
            global_participant_data[unpaired_one]['mapping'] = mapping1
            global_participant_data[unpaired_two]['mapping'] = mapping2
...
def start_interaction_trial(list_of_participants):
    ...
    if not(all_connected(list_of_participants)):
        ...
        if trial_counter>=ntrials:
            ...
            director_mapping = global_participant_data[director_id]['mapping']
...
def handle_director_response(director_id,director_response):
    ...
    if not(all_connected([matcher_id])): #the usual check that everyone is still connected
        ...
        foils = interaction_trial_list[trial_counter]['foils']
        context_array = shuffle([target]+foils)
        matcher_mapping = global_participant_data[matcher_id]['mapping']
        block_n = interaction_trial_list[trial_counter]['block']
...
def handle_matcher_response(matcher_id,matcher_response):
    ...
    if not(all_connected([director_id,matcher_id])):
        ...
        if target==guess:
            ...
            score=0
        director_mapping = global_participant_data[director_id]['mapping']
        director_feedback = {"command_type":"Feedback","score":score,
                    "target":target,"guess":guess,
                    "mapping":director_mapping,
                    "break_allowed":break_option}
        matcher_mapping = global_participant_data[matcher_id]['mapping']
        matcher_feedback = {"command_type":"Feedback","score":score,
                    "target":target,"guess":guess,
                    "mapping":matcher_mapping,
                    "break_allowed":break_option}
