...
/*
In this bs version:
- the latter blocks are between- rather than within-subjects, which 
makes the experiment shorter. Most of this is handled by the server, 
here we mainly change the instructions.
- the shapes are remapped between participants - the server sends 
over an abstract target shape, which is remapped to a client local shape
by remap_object_to_local_shape.  
*/
...
function save_al_data(data) {
  ...
  var data_to_save = [
    ...
    data.local_object_choices, //on comprehension trials, options saved as a single underscore-separated string
    data.object_selected,
    data.local_object_selected,
    data.score,
    data.rt,
  ];
  ...
  if (!HEADERS_WRITTEN) {
    ...
    var headers =
      ...
      trial_n,trial_index,time_elapsed,partner_id,\
      object,local_object,\
      label,local_label,\
      object_choices,local_object_choices,\
      object_selected,local_object_selected,\
      score,rt\n";
    line = headers + line;
  }
...
/*
This could collect more general demographics info, but at present it's just a question about 
written notes plus a comments box.
*/
function demographics(exit_mode) {
  if (exit_mode == "clean") {
    var exit_message =
      ...
      use to communicate with your partner about the different shapes, colours, objects or emotions \
      (exactly which you saw will depend on which version of the experiment you got). If you took notes, \
...
// target_object will either be a dictionary of shape and colour, or a string; if the 
// shape is an integer then it needs to be remapped onto the local shapes list 
// label_choices are integers that need to be mapped to the
// local list to be instantiated as a shape

//very simple!
function remap_integer_to_local_shape(i,mapping) {
  return mapping[i];
}

/*
an_object could be: {shape:x,colour:y} or just a string, i.e. 'pig' or 'sad'
If the latter, no remapping required.
If the former, need to check if the shape is an integer - if so, need to remap using 
remap_integer_to_local_shape - and then combine shape and colour to form a string e.g. square_white
*/
function remap_object_to_local_shape(an_object,mapping) {
  if (typeof an_object === 'string' ) {
    return an_object;
  }
  else {
    shape = an_object.shape;
    colour = an_object.colour;
    if (typeof shape == 'number') {
      shape = remap_integer_to_local_shape(shape,mapping)
    }
    return shape + "_" + colour;
  }
}

function remap_object_to_string(an_object) {
  if (typeof an_object === 'string' ) {
    return an_object;
  }
  else {
...
function director_trial(
  ...
  var local_target_object = remap_object_to_local_shape(target_object,mapping);
  var local_context_array = context_array.map(function(i) {return remap_object_to_local_shape(i,mapping)});
  var local_label_choices = label_choices.map(function(i) {return remap_integer_to_local_shape(i,mapping)});
  ...
  var single_production_trial = {
    ...
    stimulus: function() {
      var stimulus_string = ""
      for (context_item of local_context_array) {
        var style_string = "style='border:10px solid white;'"
        if (context_item==local_target_object) { //target gets a green box
    ...
    on_finish: function (data) {
      ...
      var label = label_choices[data.response];
      var local_label = local_label_choices[data.response];
      data.label = label;
      data.local_label = local_label;
      ...
      data.object = remap_object_to_string(target_object,mapping);
      data.local_object = local_target_object
      ...
      var message = {response_type: "RESPONSE",
                      ...
                      response: [label]} //this is a list for compatability with looping version
...
function matcher_trial(
  ...
  var local_director_label = director_label.map(function(i) {return remap_integer_to_local_shape(i,mapping)});
  var local_target_object = remap_object_to_local_shape(target_object,mapping);
  var local_context_array = context_array.map(function(i) {return remap_object_to_local_shape(i,mapping)});
  ...
  var trial = {
    ...
    on_finish: function (data) {
      ...
      var object_selected_to_transmit = context_array[button_number];
      ...
      data.local_label = local_director_label.join("-");
      data.object_choices = context_array.map(function(o) {return remap_object_to_string(o,mapping)}).join("_");
      data.local_object_choices = local_context_array.join("_");
      data.object_selected = remap_object_to_string(object_selected_to_transmit,mapping);
      data.local_object_selected = local_context_array[button_number];
      if (local_target_object == data.local_object_selected) {
        data.score = 1;
      } else {
        data.score = 0;
      }
      ...
      send_to_server({
        ...
        partner: partner_id,
        role: "Matcher",
        response: object_selected_to_transmit,
      });
      jsPsych.pauseExperiment();
    },
  };
...
*/
function display_feedback(director_object, matcher_object, score, mapping, break_allowed) {
  //console.log(break_allowed)
  end_waiting();

  var local_director_object = remap_object_to_local_shape(director_object,mapping);
  var local_matcher_object = remap_object_to_local_shape(matcher_object,mapping);
...
var consent_screen = {
  ...
  The full experiment takes 25-35 minutes to complete and you will be paid &pound;5.</p>\
  \
  ...
  <a href='shapes_participant_information_bs.pdf' download>(pdf)</a> \
